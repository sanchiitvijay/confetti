...
const Stats = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribe1 = onSnapshot(
      ...
      (err) => {
        console.log(
          "Error while listening to real time updates for top posts",
          err
        );
      }
    );
  ...
  return (
    <Suspense fallback={<Loader />}>
      <div className="text-black dark:text-white text-center w-[90%]">
        <p className="text-4xl m-7 animate-text bg-gradient-to-r from-teal-500 via-purple-500 to-orange-500 bg-clip-text text-transparent font-black ">
          Leaderboard
        </p>
        <div className="flex md:flex-row-reverse flex-col-reverse gap-5 items-center justify-evenly">
          ...
          <div className="bg-gradient-to-r w-[75%] lg:w-[32%] gap-y-5 mb-5 from-pink-500 via-red-500 to-yellow-500 background-animate gradient-box p-1 rounded-md">
            <div className="rounded-md p-4 bg-white dark:bg-confettiDarkColor3">
              ...
              <div className="grid place-content-center place-items-start mx-auto">
                {/* map isme lgana hai */}
                {likeCount?.slice(0, 5)?.map((like, index) => (
                  <div className="flex  gap-2 flex-row mt-5" key={index}>
                    {index === 0 ? (
                      <img
                        src={gold}
                        alt="gold"
                    ...
                    <div className="gap-4 flex flex-row">
                      <img
                        src={like?.dp}
                        alt={like?.author}
                        className="w-[50px] h-[50px] rounded-full border-black dark:border-white border"
                      />
            ...
            <div className="rounded-md p-4 bg-white dark:bg-confettiDarkColor3">
              ...
              <div className="grid place-content-center place-items-start mx-auto">
                ...
                {postCount?.slice(0, 5)?.map((post, index) => (
                  <div
                    className="flex place-content-center gap-2 flex-row mt-5"
                    key={index}
                  >
                    {index === 0 ? (
                      <img
